...
void seatest_test_fixture_end_details()
{
	char s[SEATEST_PRINT_BUFFER_SIZE];
	sprintf(s, "[Total] %d run, %d failed", sea_tests_passed, sea_tests_failed);
...
{
    seatest_test_fixture_end_details();
	char s[SEATEST_PRINT_BUFFER_SIZE];
	sprintf(s, "%d run  %d failed Total", sea_tests_run-seatest_fixture_tests_run, sea_tests_failed-seatest_fixture_tests_failed);
	seatest_header_printer(s, seatest_screen_width, ' ');
	if(seatest_is_display_only() || seatest_machine_readable) return;
	printf("\r\n");
}
